...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  const chevron = (direction: "left" | "right") => (
    <svg className="fan-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div className="fan-section">
      <div className="fan-section-inner">
        <div ref={containerRef} className="fan-layout">
          {cards.map((card, index) => {
            const image = (
              <div className="fan-card-image-wrapper">
                <img src={card.imgUrl} loading="lazy" alt={card.alt || `Card ${index}`} className="fan-card-image" />
              </div>
            );
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card fan-card-link">{image}</a>
            ) : (
